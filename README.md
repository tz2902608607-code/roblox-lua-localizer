...
- **18 种语言**可选，自由设定源语言和目标语言
- 生成 Roblox Lua 汉化脚本（汉化模板 v3：精确匹配 + 部分替换双模式，Hook / 普通翻译两种模式）
- 支持远程脚本 Raw 链接
- 支持 JSON 导入和导出
- 支持复制代码和下载 `.lua` 文件
- 内置使用教程 + 翻译接口说明
...
源语言支持"自动检测"，目标语言需明确指定。目前支持以下 18 种语言：
...
- 支持精确匹配 + 部分替换双模式翻译
- 支持 Hook 翻译和普通翻译两种模式
- 更好的 SystemUI 过滤，避免误翻译系统界面
- 外部脚本加载集成到生成脚本中

### v3.1.0（2026-07-03）

- 新增翻译语言选择功能，支持 18 种语言，自由选择源语言和目标语言
- 所有翻译接口均支持自定义语言对
- 修复"申请 Key"按钮对齐问题
- 重写使用教程，以手动翻译为主、自动翻译为辅
- 新增翻译接口说明页面
...
- 新增网站已运营时长显示（自动计算，实时更新）

### v2.9.0（2026-07-02）

- 每个需 Key 的翻译接口新增"申请 Key"按钮
- 新增 note 提示（如百度大模型与百度翻译共用 Key）
- 重新编写 README 文档

### v2.8.0（2026-07-01）

- Cloudflare Turnstile 人机验证通过后 3 秒自动隐藏
...
- 新增 5 个国内 AI 大模型翻译接口：通义千问、智谱AI GLM、讯飞星火、零一万物 Yi、腾讯混元
...
- 新增百度AI 翻译接口（文心一言 ERNIE Speed，免费）
- AI 翻译接口支持自定义提示词
...
- 重构翻译架构，支持 Cloudflare Function 代理
- 新增 15+ 翻译接口
